...
const categoryImages: Record<string, string> = {
  ...
  "Family & People":
    "https://images.unsplash.com/photo-1511895426328-dc8714191300?w=400&h=300&fit=crop&crop=face",
  "Time & Numbers":
  ...
  proper_noun:
    "https://images.unsplash.com/photo-1477414956199-7dafc86a4f1a?w=400&h=300&fit=crop",
  preposition:
    "https://images.unsplash.com/photo-1495364141860-b0d03eccd065?w=400&h=300&fit=crop",
  interjection:
    "https://images.unsplash.com/photo-1464207687429-7505649dae38?w=400&h=300&fit=crop",

  // Legacy and fallback categories - all unique
  greetings:
    "https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=400&h=300&fit=crop&crop=face",
  numbers:
  ...
  adjectives:
    "https://images.unsplash.com/photo-1532153354603-2ba9420952f0?w=400&h=300&fit=crop",
  general:
...
export function CategoryList({
  ...
  const getWordCount = (categoryId: string) => {
    ...
    let words = vocabularyWords.filter(
      (word) => word.categories && word.categories.includes(categoryId)
    );

    // Filter by difficulty level if not 'all'
    if (selectedDifficulty !== "all") {
      // Map selectedDifficulty to CEFR range
      const cefrRange = {
        beginner: ["A1", "A2"],
        intermediate: ["B1", "B2"],
        advanced: ["C1", "C2"],
    ...
    <div className="max-w-md mx-auto p-4">
      <div className="grid grid-cols-2 gap-3">
        {categories.map((category, index) => {
          ...
          const imageUrl =
            categoryImages[category.id] || categoryImages.general;
            ...
            >
              ...
              <div className="p-3">
                <h3 className="text-sm font-semibold text-gray-900 text-left leading-tight group-hover:text-gray-700 transition-colors mb-2">
                  {categoryTranslations[language][category.name] ||
                    category.name}
                </h3>
                      ...
                      width:
                        wordCount > 0
                          ? `${Math.min(
                              (wordCount /
                                Math.max(
                                  ...categories.map((c) => getWordCount(c.id))
                                )) *
                                100,
                              100
                            )}%`
